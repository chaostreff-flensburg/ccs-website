...
app.post("/api/form", async (req, res) => {
  ...
  const requiredFieldsMissing = requiredFields.filter((element) =>
    !req.body?.[element]
  );
  ...
  try {
    ...
    if (
      e.response.data.includes("Email address is already used for other user.")
    ) {
      ...
      res.status(500);
      res.send(
        "Beim Erstellen des Benutzers ist ein Fehler aufgetreten. Wenn der Fehler noch mal auftritt, solltest du dich an ccs@chaostreff-flensburg.de melden.",
      );
      return;
    }
  }
  ...
  try {
    ...
    res.status(500);
    res.send(
      "Beim Erstellen des Tickets ist ein Fehler aufgetreten. Wenn der Fehler noch mal auftritt, solltest du dich an ccs@chaostreff-flensburg.de melden.",
    );
  }
});
